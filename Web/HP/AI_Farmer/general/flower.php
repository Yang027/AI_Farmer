...
<body>
  ...
  if (isset($_POST["find"])) {
    ...
    $query .= " WHERE classification='花卉' and plant_id like '%" . $_POST["find"] . "%'";
  ...
  <div class="row line1 justify-content-center my-5">
    <form class="row" method="post" action="flower.php">
    ...
    <div class="filter-btns shadow-md rounded-pill text-center col-auto">
      ...
      <a class="filter-btn btn rounded-pill btn-outline-primary border-0 m-md-2 px-md-4 active" data-filter=".social" href="flower.php">花卉</a>
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    while ($i < mysqli_num_rows($result)) {
      ...
      while ($j < 3) {
        ...
        if ($i <= mysqli_num_rows($result)) {
          ...
          echo '<a href="flower_info.php?item=' . $row['plant_id'] . '"><button class="button1">Read more <img class="vertical-align:middle;" src="../icon/click.png" height="25" width="25"></button></a>';
          echo '</div>';
          echo '</div>';
          echo '</div>';
          echo '</div>';
    ...
    $query = "SELECT * FROM plant WHERE classification='花卉'"; //搜尋 *(全部欄位) ，從 表staff
    ...
    $offset = ($page - 1) * $num;
    $sqll = "SELECT * FROM plant WHERE classification='花卉' LIMIT $offset , $num";
    ...
    echo '</div>';
    echo ' <h5 style="text-align:center; color:#98814C;>';
    echo  '<a href="flower.php?page=1">首頁 </a>';
    echo '<a href="flower.php?page=' . ($page - 1) . '"><font color="#98814C">上一頁</a>';
    echo ' 當前是第 ' . $page . ' 頁 ';
    echo '<a href="flower.php?page=' . ($page + 1) . '"><font color="#98814C">下一頁  </a>';
    echo '  共 ' . $total . ' 頁 ';
    echo '<a href="flower.php?page=' . $total . '"><font color="#98814C">尾頁</a>';
    echo '</h5>';
  }
  $con->close();
  ?>
