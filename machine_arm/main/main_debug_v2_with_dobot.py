...
def Rail(state,api,L_bias):
    if (state == dType.DobotConnect.DobotConnect_NoError):
        ...
        # return L_ori
def arm_Z(state,api,Z_bias,X_bias):
    if (state == dType.DobotConnect.DobotConnect_NoError):
        dType.SetQueuedCmdClear(api)
...
def run(pipeline,alignedFs,state,api):
    try:
        while True:
            ...
            if mature_boxs.any(): # mature_boxs.any()
                ...
                #     dist = get_mid_pos(color_frame, boxs[i], depth_image, 24)
                #     X_can_pick_max_distance = (boxs[i][1] / 19.28571428571429) + 29 if boxs[i][1] <= 135 else 38 - ((boxs[i][1] - 135) / 21.85714285714286)
                #     if (dist/10) > X_can_pick_max_distance or np.isnan(dist) or boxs[i][1] > 290:
                #         unpickable.append(i)
                ...
                # 2.計算成熟番茄與中心點的直線距離
                    ...
                    if L_bias > 5 or L_bias < -5:
                        # print("L移動值-->",L_bias)
                        # 4.移動手臂進行剪取
                        Rail(state,api,L_bias)
                        #
                        # print("L的位置 -->",L_position)
                        # print("物體的位置 -->",(int(box[2])+int(box[0]))/2)
                    else:
                        # if ((int(box[2])+int(box[0]))/2) < 343+5 and ((int(box[2])+int(box[0]))/2) > 343-5:
                        Z_bias = 0 - float(box[1])*0.60
                        # X_can_pick_max_distance =7-((STUDY_UP['Z'] + Z_bias)/0.5925925925925926) / 19.28571428571429 +29 if box[1] < 135 else 36+((STUDY_UP['Z'] + Z_bias)/0.8496732026143791) / 21.85714285714286-7
                        ...
                        # if dist/10 < X_can_pick_max_distance and X_bias < 325:
                        arm_Z(state,api,Z_bias,X_bias)
                            # print("採摘")
                            # print(f'{dist/10} cm')
                        # break
                else:
...
if __name__ == "__main__":
    ...
    if (state == dType.DobotConnect.DobotConnect_NoError):
        ...
        # wait_move = dType.SetWAITCmd(api, 2000, isQueued=1)[0]
        dType.SetQueuedCmdStartExec(api)
        while wait_move > dType.GetQueuedCmdCurrentIndex(api)[0]:
            dType.dSleep(100)
        dType.SetQueuedCmdStopExec(api)
        run(pipeline,alignedFs,state,api)
        dType.DisconnectDobot(api)
        print('exit')
